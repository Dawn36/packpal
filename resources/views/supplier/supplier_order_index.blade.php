            ...
            <div class="card mb-5 mb-xl-10">
                <div class="card-body ">
                    ...
                    <ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bolder">
                        ...
                        <!--begin::Nav item-->
                        <li class="nav-item">
                            <a class="nav-link text-active-primary py-5 me-6 position-relative {{$inactive}}" href="{{ route('supplier_order_status', ['status' => 'inprocess']) }}">In-Process<span class="position-absolute top-100 start-100 translate-middle  badge badge-circle badge-info">{{$orderStatusCount[0]->inprocess == null ? '0' : $orderStatusCount[0]->inprocess}}</span></a>
                        </li>
                        <!--end::Nav item-->
                        <!--begin::Nav item-->
                        <li class="nav-item">
                                        ...
                                        <table class="kt_datatable_example_1 table table-row-bordered gy-5">
                                            ...
                                            <tbody>
                                                @for ($i = 0; $i < count($order); $i++) @php $a=$i; $a++; @endphp <tr>
                                                    ...
                                                    <td>
                                                        @if($title == 'Offer')
                                                        <form style="display: inline-block" method="POST" action="{{ route('order_delete', $order[$i]->id) }}">
                                                            @csrf
                                                            <button type="submit" class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-danger" data-bs-toggle="tooltip" data-bs-original-title="Delete Offer">
...
<script>
    ...
    function editOffer(orderId,sUserId,bUserId) {
        var value = {
            orderId: orderId,
            sUserId: sUserId,
            bUserId: bUserId,
        ...
        $.ajax({
            ...
            data: value,
            success: function(result) {
                $('#myModalLgHeading').html('Give new offer');
