<form id="" class="form" method="POST" action="{{ route('ads.update',$ads->id) }}" enctype="multipart/form-data">
    @method("PUT")
    @csrf
    <div class="d-flex flex-column scroll-y me-n7 pe-7" id="" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_add_user_header" data-kt-scroll-wrappers="#kt_modal_add_user_scroll" data-kt-scroll-offset="300px">
        <div class="fv-row mb-7">
            <label class="d-block fw-bold fs-6 mb-5">Avatar</label>
            <div class="image-input image-input-outline" data-kt-image-input="true" style="background-image: url({{asset($ads->path)}})">
                <div class="image-input-wrapper w-125px h-125px" style="background-image: url({{asset($ads->path)}});">
                </div>
        ...
        <div class="fv-row mb-7">
            <label class="required fw-bold fs-6 mb-2">Ads Name</label>
            <input type="text" name="ads_name" value="{{$ads->ads_name}}" class="form-control form-control-solid mb-3 mb-lg-0" placeholder="Please Enter ads name." required />
        </div>
        <div class="fv-row mb-7">
            <label class="form-label required">Status</label>
            <select class="form-control required form-control-lg form-control-solid" id='status' name="status" required >
                <option value="">Select status</option>
                <option value="top" {{$ads->status == 'top' ? 'Selected' : ''}} >Top</option>
                <option value="middle" {{$ads->status == 'middle' ? 'Selected' : ''}}>Middle</option>
                <option value="last" {{$ads->status == 'last' ? 'Selected' : ''}}>Last</option>
                
            </select>
            <div class="text-muted fs-7 mb-7">Select Status.</div>
        </div>
