...
<div class="toolbar py-5 py-lg-15" id="kt_toolbar">
    ...
    <div id="kt_toolbar_container" class="container-xxl d-flex flex-stack flex-wrap">
        ...
        <div class="page-title d-flex flex-column me-3">
            <!--begin::Title-->
            <h1 class="d-flex text-white fw-bolder my-1 fs-3">Ads</h1>
            <!--end::Title-->
            <!--begin::Breadcrumb-->
            <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
                ...
                <!--begin::Item-->
                <li class="breadcrumb-item text-white opacity-75">
                    <a href="" class="text-white text-hover-primary">Ads Listing</a>
                </li>
        ...
        <div class="d-flex align-items-center py-3 py-md-1">
            ...
            <div class="me-4">
                <!--begin::Menu-->
                <button type="button" class="btn btn-custom btn-active-white btn-flex btn-color-white btn-active-color-primary fw-bolder" data-bs-original-title="Create a new product" onclick="addads()">
                    ...
                    <div class="card card-docs mb-2">
                        <div class="p-10">
                            ...
                            <h1 class="anchor fw-bolder mb-5" id="zero-configuration">
                                <a href="javascript:;"></a>Ads Listing
                            </h1>
                            <!--begin::Notice-->
                            <div class="d-flex align-items-center rounded py-5 px-4 bg-light-dark">
                                ...
                                <!--end::Icon-->
                                <!--begin::Description-->
                                <div class="text-gray-700 fw-bold fs-6 lh-lg">Here we have a list of all the ads that we have.</div>
                                <!--end::Description-->
                            </div>
                            <!--end::Notice-->
                            <!--end::Heading-->
                            ...
                            <div class="py-5">
                                <table class="kt_datatable_example_1 table table-row-bordered gy-5">
                                    ...
                                    <tbody class="fw-bold text-gray-600">
                                        @for ($i = 0; $i < count($ads); $i++) @php $a=$i; $a++; @endphp <tr>
                                            <td>{{$a}}</td>
                                            <td>{{ucwords($ads[$i]->ads_name)}}</td>
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <div class="me-5 position-relative">
                                                        ...
                                                        <div class="symbol symbol-35px symbol-circle">
                                                           <img alt="Pic" src="{{asset($ads[$i]->path)}}" >
                                                        </div>
                                                        <!--end::Avatar-->
                                                    </div>
                                                </div>
                                            ...
                                            <td>{{ucwords($ads[$i]->status)}}</td>
                                            <td>{{Date('Y-m-d',strtotime($ads[$i]->created_at))}}</td>
                                            <td>
                                                <button type="button" class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-primary" data-bs-original-title="Edit product" onclick="editAds('{{$ads[$i]->id}}')">
                                                ...
                                                <form style="display: inline-block" method="POST" action="{{ route('ads.destroy', $ads[$i]->id) }}">
                                                    @method('DELETE')
                                                    @csrf
                                                    <button type="submit" class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-danger me-2" data-bs-toggle="tooltip" data-bs-original-title="Delete ads">
...
<script>
    ...
    function addads() {
        ...
        $.ajax({
            type: 'GET',
            url: "{{ route('ads.create') }}",
            success: function(result) {
                $('#myModalLgHeading').html('Add Ads');
    ...
    function editAds(id) {
        url = "{{ route('ads.edit', ':id') }}";
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            success: function(result) {
                $('#myModalLgHeading').html('Edit Ads');
                $('#modalBodyLarge').html(result);
                $('#myModalLg').modal('show');
            }
        });
