        ...
        <div class="container">
            ...
            @for ($i = 0; $i < count($catAndSubCat); $i++)
                ...
                <div class="row" style="margin-bottom: 20px">
                    <div class="col-lg-3">
                        <div class="service">
                            <a href="{{ route('web_supplier_listing', ['category_id' => $catAndSubCat[$i]->cat_id]) }}"><img
                                    src="{{ asset('category/' . $catAndSubCat[$i]->category_image) }}" /></a>
                            <h3 style="color: #ff6202">{{ ucwords($catAndSubCat[$i]->category_name) }}</h3>
                        </div>
                    </div>
                    <div class="col-lg-9">
                        @for ($j = 0; $j < count($dataSubId); $j++)
                            <a href='{{ route('web_supplier_listing', ['category_id' => $catAndSubCat[$i]->cat_id, 'sub_category_id' => $dataSubId[$j]]) }}'
                                type="button" class="btn btn-outline-success btn-sm" style="margin-bottom: 10px">
