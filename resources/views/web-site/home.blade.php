...
@section('content_website')
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="main-search-headlines">
                            ...
                            <p style="font-style: italic;">We know how desperate brands/people are in knowing different
                                packaging that exists in the market.</p>
                            <p style="font-style: italic;">We like Packaging Companies especially in Pakistan to prosper and
                                grow itself intensively and broader by giving them an economical platform to opt for.</p>
                ...
                <div class="msps-slider-container">
                    <div class="msps-slider">
                        @for ($i = 0; $i < count($adsTop); $i++)
                            <div class="item">
                                <img src="{{ asset($adsTop[$i]->path) }}" class="item"
                                    title="{{ asset($adsTop[$i]->path) }}" />
                            </div>
                        @endfor
                    </div>
                </div>
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-white osahan-nav-mid px-0 border-top shadow-sm">
        <div class="container">
            ...
            <div class="collapse navbar-collapse scroll-color" id="navbarResponsive">
                <ul class="navbar-nav">
                    @for ($i = 0; $i < 7; $i++)
                        {{-- @php
                        ...
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle" href="#"
                                id="navbarDropdownPortfolio{{ $i }}" data-toggle="dropdown" aria-haspopup="true"
                                aria-expanded="false">

                                @php
                                    $categoryName = explode(' ', $categories[$i]->category_name, 3);
                                @endphp
                                @if (isset($categoryName[2]))
                                    <b> {{ ucwords($categoryName[0]) }} {{ ucwords($categoryName[1]) }} <br>
                                        {{ ucwords($categoryName[2]) }}</b>
                                @else
                                    <b>{{ ucwords($categories[$i]->category_name) }}</b>
                                @endif

                            </a>
                            <div class="dropdown-menu" aria-labelledby="navbarDropdownPortfolio{{ $i }}">
                                @php
                    ...
                    <button onclick="viewAllCategory()" type="button" class="btn btn-outline-success btn-sm" tabindex="0"
                        style="border: none; margin-top: 10px;height: 50%;">
                        View All
                    </button>
                </ul>
            </div>
                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="view recent-slider recommended-slider">
                            @for ($i = 0; $i < count($supplier); $i++)
                                ...
                                <div>
                                    <a
                                        href="{{ route('supplier_detail', ['userId' => $supplier[$i]->user_id, 'catId' => $supplier[$i]->cat_id]) }}">
                                        <img class="img-fluid"
                                            src="{{ asset('/profile/' . $supplier[$i]->company_banner) }}" />
                                    </a>
                                    <div class="inner-slider"
                                        onclick="location.href = '{{ route('supplier_detail', ['userId' => $supplier[$i]->user_id, 'catId' => $supplier[$i]->cat_id]) }}';"
                                        ...
                                        <div class="inner-wrapper">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <span class="seller-name">
                                                    <a
                                                        href="{{ route('supplier_detail', ['userId' => $supplier[$i]->user_id, 'catId' => $supplier[$i]->cat_id]) }}">{{ charaterCountTo20($supplier[$i]->first_name . ' ' . $supplier[$i]->middle_name . ' ' . $supplier[$i]->last_name) }}</a>
                                                    <span class="level" style="font-weight: bold; color: #524e4e">
                                                    ...
                                                    <span class="level" style="font-weight: bold; color: #524e4e">
                                                        Sub Category:
                                                        {{ charaterCountTo20($supplier[$i]->sub_category_name) }}
                                                    </span>
                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="view recent-slider recommended-slider">
                            @for ($i = 0; $i < count($bidListing); $i++)
                                <div onclick="location.href = '{{ route('bid_detail', $bidListing[$i]->bid_id) }}';"
                                    style="cursor: pointer;">
                                    ...
                                    <div class="inner-slider">
                                        <div class="inner-wrapper">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <span class="seller-name">
                                                    <a href="{{ route('bid_detail', $bidListing[$i]->bid_id) }}">{{ charaterCountTo20($bidListing[$i]->first_name . ' ' . $bidListing[$i]->middle_name . ' ' . $bidListing[$i]->last_name) }}
                                                    </a>
                                                    <span class="level" style="font-weight: bold; color: #524e4e">
                                                    ...
                                                    <span class="level" style="font-weight: bold; color: #524e4e">
                                                        Sub Category:
                                                        {{ charaterCountTo20($bidListing[$i]->sub_category_name) }}
                                                    </span>
                                                    <span class="level" style="font-weight: bold; color: #524e4e">
                                                        Buyer from:
                                                        {{ charaterCountTo20($bidListing[$i]->city . ' - ' . $bidListing[$i]->country) }}
                                                    </span>
                                                    <a
                                                        href="{{ route('bid_detail', $bidListing[$i]->bid_id) }}">{{ charaterCountTo26($bidListing[$i]->bids_name) }}</a>
                                                </span>
                                            </div>
                                                ...
                                                <div class="price">
                                                    @if (Auth::check())
                                                        @if (date('Y-m-d') > Auth::user()->expiry_date && Auth::user()->hasRole('supplier'))
                                                            <a href="{{ route('subscribe_page') }}" type="button"
                                                                class="btn btn-outline-success btn-sm">Bid Now</a>
                                                        @else
                                                            <a href="{{ route('bid_detail', $bidListing[$i]->bid_id) }}"
                                                                type="button" class="btn btn-outline-success btn-sm">Bid
                                                                Now</a>
                                                        @endif
                                                    @else
                                                        <a href="{{ route('bid_detail', $bidListing[$i]->bid_id) }}"
        ...
        <div class="container view_slider recommended">
            ...
            <div class="row service-slider">
                @for ($i = 0; $i < count($categories); $i++)
                    <div class="col">
                        <div class="service">
                            <a href="{{ route('web_supplier_listing', ['category_id' => $categories[$i]->cat_id]) }}">
                                <h3 style="color: #ff6202">{{ ucwords($categories[$i]->category_name) }}</h3>
                                <img src="{{ asset('category/' . $categories[$i]->category_image) }}"
                                    alt="{{ $categories[$i]->category_name }}" />
