                        ...
                        <form method="GET" action="{{ route('web_supplier_listing') }}">
                            <div class="dropdown-filters d-flex">
                                <select id='category_id' name="category_id" class="form-control"
                                    onchange="getSubCategoryAjax()">
                                    <option value="">Select Main Category</option>
                                    @for ($i = 0; $i < count($categories); $i++)
                                        <option value="{{ $categories[$i]->id }}"
                                            {{ $categoryId == $categories[$i]->id ? 'selected' : '' }}>
                                            {{ ucwords($categories[$i]->category_name) }}</option>
                                    @endfor
                                </select>
                ...
                <div class="sorting-div d-flex align-items-center justify-content-between">
                    ...
                    <div class="sorting d-flex align-items-center" style="margin-bottom: 20px;margin-right: 7px;">
                        ...
                        <form id='newold' method="GET" action="{{ route('web_supplier_listing') }}">
                            <select class="custom-select custom-select-sm border-0 shadow-sm ml-2" name="newold"
                                onchange="AscDesc()">
                                ...
                                <div class="inner-wrapper">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <span class="seller-name">
                                            <a
                                                href="{{ route('supplier_detail', ['userId' => $supplier[$i]->user_id, 'catId' => $supplier[$i]->cat_id]) }}">{{ ucwords($supplier[$i]->first_name) }}
                                                {{ ucwords($supplier[$i]->last_name) }}</a>
